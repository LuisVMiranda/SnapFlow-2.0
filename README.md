...
Se o PostgreSQL estiver `healthy` dentro do container, mas a porta local estiver ausente ou continuar inacessível após a espera, o iniciador tenta **uma única recuperação**: recria somente `snapflow-postgres`, reutilizando o volume nomeado já identificado, e repete a verificação SQL por até 90 segundos. Não reinicia todo o Docker/WSL, não remove volumes e não mexe em containers avulsos. Falhas de autenticação, banco `unhealthy` ou ainda em recuperação não acionam essa tentativa. Caso a porta continue inacessível, confira o log e reinicie o Docker Desktop manualmente.
