...
- Start the API, panel and website together with `INICIAR_TUDO.bat`. It first force-stops previously identified SnapFlow processes on the configured ports, then starts the three services as hidden background processes, so no extra terminal windows remain open; inspect `logs\\api.log`, `logs\\panel.log` and `logs\\website.log` if needed.
...
| Four/three/one portrait carousel | Unit/property tests and measured browser layouts across desktop, tablet, phone and landscape widths; four desktop slots remain fixed with fewer galleries, which are centered without enlargement. The strip fills the width when height permits, shrinking on short screens to keep the 2:3 covers and breathing room visible. Overlay arrows, smooth one-card transitions, no horizontal overflow, keyboard and swipe logic. The help link uses a readable 1rem font. |
