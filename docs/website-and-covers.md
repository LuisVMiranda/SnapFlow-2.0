...
| Five/three/one portrait carousel | Unit/property tests and measured browser layouts across desktop, tablet, phone and landscape widths; five desktop slots remain fixed with fewer galleries, which are centered without enlargement. The strip fills the width when height permits, shrinking on short screens to keep the 2:3 covers and breathing room visible. Overlay arrows, smooth one-card transitions, no horizontal overflow, keyboard and swipe logic |
